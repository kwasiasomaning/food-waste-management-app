...
/**
 * Expo is bound to IPv4 0.0.0.0:8081. Cursor Browser often hits ::1 first
 * (localhost resolves IPv6 first) and gets ERR_CONNECTION_REFUSED.
 * Forward IPv6 loopback to IPv4 loopback on the same port.
 */
const net = require('net');

const port = Number(process.env.EXPO_PORT || process.env.PORT || 8081);
